...
<html lang="en">
  ...
  <?php 
	  try {
		$auth = new PDO('mysql:host=localhost;dbname=mini_project', 'root', '');
	  }
	  catch(Exception $error) {
		die('Error : ' . $error->getMessage());
	  }

	  $sql = "SELECT id, title, description_, start_, end_, color FROM events ";
...
$(function() {
   ...
   $('#calendar').fullCalendar({
	   ...
	   nowIndicator: true,
	   now: '2021-12-11T11:15:00', //remove - only for demo
	   scrollTime: '08:00:00',
	   defaultDate: '2021-12-07', // Use this line to use the current date: new Date(),
	   editable: true,
	   navLinks: true,
	   ...
	   <?php foreach($events as $event): 
		   ...
		   $start = explode(" ", $event['start_']);
		   $end = explode(" ", $event['end_']);
		   if($start[1] == '00:00:00'){
			   $start = $start[0];
		   }else{
			   $start = $event['start_'];
	   ...
	   ?>
		   {
			   ...
			   description: '<?php echo $event['description_']; ?>',
