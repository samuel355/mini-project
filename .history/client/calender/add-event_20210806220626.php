<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST')
  {
    ...
    $subject_ = 'New Event';
    $message_ = 'Customer added new event to calender';
    ...
    if(empty($title)){
      ...
      try{
        ...
        $insert2 = "INSERT INTO messages (firstname, email, subject_, message_, phone) VALUES (:firstname, :email, :subject_, :message_, :phone)";
        
        $statement1 = $connect->prepare($insert1);

        $statement2 = $connect->prepare($insert2);

        $check1 = $statement1->execute([
          ':title' => $title,
          ':email' => $email,
          ':start_' => $start_,
          ':end_' => $end_,
