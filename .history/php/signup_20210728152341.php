...
if(isset($_GET['action'])  == 'register'){
    ...
    elseif(!preg_match("#[A-Z]+#",$_["password"])) {
        http_response_code(400);
    ...
    }elseif ($_["password"] !== $_["repassword"]){
        http_response_code(400);
        echo json_encode("Your Passwords do not match");
    }else{
        $firstname = trim($_['firstname']);
        $lastname = trim($_['lastname']);
        $username = trim($_['username']);
        $email = trim($_['email']);
        $password = trim($_['password']);
        $password = PASSWORD_HASH($_['password'],PASSWORD_DEFAULT);
        $address = trim($_['address']);

        $registered_on = date("M n, Y") ." At ". date("h:i A");

        //auto generated fields
        $reg_code = substr($_['firstname'],0,2).date('y').rand(0000,9999);
